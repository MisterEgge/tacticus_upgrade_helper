...
{key:"recipients",label:"Recipients",sort:r=>r.topPriority,search:r=>r.recipients.map(x=>x.character).join(" "),render:r=>r.recipients.length?<>{r.recipients.slice(0,3).map((x,index)=><div key={`${x.character}-${x.priority}-${index}`}><strong>{x.character}</strong></div>)}{r.recipients.length>3?<small>+{r.recipients.length-3} more</small>:null}</>:"—"},
