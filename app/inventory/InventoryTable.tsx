...
{key:"recipients",label:"Recipients",sort:r=>r.topPriority,search:r=>r.recipients.map(x=>x.character).join(" "),render:r=>r.recipients.length?<>{r.recipients.slice(0,3).map(x=><div key={x.character}><strong>{x.character}</strong><small>Priority {x.priority}</small></div>)}{r.recipients.length>3?<small>+{r.recipients.length-3} more</small>:null}</>:"—"},
