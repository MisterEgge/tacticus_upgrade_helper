...
import { campaignRankIndex, campaignRecommendationPriority, requiredCampaignName } from "../../src/domain/campaigns";
...
export default async function Farming({ searchParams }: { searchParams: Promise<Query> })
{
    ...
    try
    {
        ...
        const campaignGoalCandidates = query.plan === "campaign" ? Object.entries(campaignTargets.campaigns).flatMap(([campaignName, campaign]) => { const campaignProgress = report.campaignProgress?.find(c => (c.type === "Elite" || c.type === "EliteMirror") && requiredCampaignName(c) === campaignName); const campaignComplete = (campaignProgress?.highestCompletedBattle ?? 0) >= 40; return catalog.characters.filter(character => character.campaignsRequiredIn.includes(campaignName)).flatMap(character => { const unit = rankRoster.find(u => u.id === character.id); const target = campaign.characters[character.name]; if (!unit || !target?.rank) return []; const recommendation = campaignRecommendationPriority({ campaign: campaignName, campaignComplete, characterId: character.id, characterName: character.name, currentRank: unit.rank, targetRank: target.rank, role: target.role, confidence: target.confidence, accountPriority: priorities[character.name]?.priority ?? 0 }); const targetRank = campaignRankIndex(target.rank); return recommendation.recommendationPriority > 0 && targetRank !== null ? [{ id: unit.id, name: unit.name, currentRank: unit.rank, targetRank, ...(unit.upgrades === undefined ? {} : { upgrades: unit.upgrades }), priority: recommendation.recommendationPriority, ...(target.confidence === undefined ? {} : { confidence: target.confidence }) }] : []; }); }) : [];
