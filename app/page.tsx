...
export default async function Home()
{
    ...
    return <main>
        ...
        <section className="cards overviewCards">
            ...
            <Card label="Priority characters" value={focus.length} detail="need gear or ability attention" href="/characters"/>
        ...
        <section className="panel overviewPanel"><div className="sectionTitle"><div><p className="eyebrow">WHERE TO FOCUS</p><h2>Highest-priority characters</h2><p className="sub">Each character appears once. Gear and ability work are combined so you can choose who to invest in next.</p></div><Link className="viewLink" href="/characters">View character roster →</Link></div>
            {top.length ? <div className="tableWrap"><table><thead><tr><th>Character</th><th>Upgrade focus</th><th>Priority</th><th>Plan</th></tr></thead><tbody>{top.map(row =>
                {
                    ...
                    </td><td>{row.priority}</td><td><Link className="viewLink" href={unit ? `/characters/${encodeURIComponent(unit.id)}` : "/characters"}>Open character →</Link></td></tr>;
