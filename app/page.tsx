...
export default async function Home() {
  ...
  return <main>
    ...
    {top.map(row=>{const now=report.equipmentAllocation.equipNow.includes(row);return <tr key={row.character+row.slotId}><td><CharacterName name={row.character}/><small>{row.slotId}</small></td><td>{row.currentItem}</td><td><strong>{targetName(row)}</strong></td><td><span className={"status "+(now?"ready":"need")}>{now?"EQUIP NOW":"NEED"}</span></td></tr>})}
