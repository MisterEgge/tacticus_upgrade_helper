...
export default async function GuildRaid()
{
    ...
    const [report,catalog,metaText,guidance]=await Promise.all([getReport(),getCharacterCatalog(),readFile("config/raid_boss_meta.json","utf8"),getAbilityBreakpoints()]);
    ...
    const teams=Object.fromEntries(Object.entries(meta.bosses).map(([boss,options])=>[boss,Object.fromEntries(Object.entries(options).map(([name,team])=>[name,{...team,members:[...team.core,...team.flex].map(name=>{const unit=units.get(name);const target=guidance[name] as {active?:{practical?:string};passive?:{practical?:string}}|undefined;return {name,id:unit?.id??catalog.characters.find(character=>character.name===name)?.id??name,icon:unit?icons.get(unit.id):undefined,owned:!!unit,rank:unit?.rank??null,xpLevel:unit?.xpLevel??null,activeLevel:unit?.abilities[0]?.level??null,passiveLevel:unit?.abilities[1]?.level??null,activeTarget:target?.active?.practical??"Research needed",passiveTarget:target?.passive?.practical??"Research needed"};})}]))]));
