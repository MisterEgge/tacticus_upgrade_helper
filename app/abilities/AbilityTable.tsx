"use client";
import CharacterName from "../components/CharacterName";import DataTable,{Column} from "../components/DataTable";import type {AbilityRow,Report} from "../lib/report";
...
export default function AbilityTable({rows}:{rows:Row[]}){
 ...
 {key:"focus",label:"Focus",render:x=>x.focus,sort:x=>x.focus,search:x=>x.focus},
 {key:"basis",label:"Basis",render:x=><small>{x.basis}</small>,search:x=>x.basis}
 ];return <DataTable rows={rows} columns={cols} placeholder="Search character, faction, ability or focus…"/>;
}