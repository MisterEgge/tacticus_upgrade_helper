...
export default function AbilityTable({rows,campaignIds,raidMeta}:{rows:Row[];campaignIds:string[];raidMeta:RaidMetaFile}){
 ...
 return <><div className="abilityViews"><label>View<select value={view} onChange={event=>setView(event.target.value)}><option value="all">All owned characters</option><option value="campaign">Campaign characters</option><option value="raid">Guild Raid meta team</option></select></label>{view==="raid"?<><label>Raid boss<select value={boss} onChange={event=>{const next=event.target.value;setBoss(next);setTeam(Object.keys(bosses[next]!)[0]!);}}>{Object.keys(bosses).map(name=><option key={name}>{name}</option>)}</select></label><label>Team<select value={team} onChange={event=>setTeam(event.target.value)}>{Object.keys(bosses[boss]!).map(name=><option key={name}>{name}</option>)}</select></label></>:null}</div>{view==="raid"?<section className="raidTeamSummary"><div><small>CORE · {selected.core.length-missingCore.length}/{selected.core.length} OWNED</small><strong>{selected.core.join(" · ")}</strong></div>{missingCore.length?<div><small>MISSING CORE</small><span>{missingCore.join(" · ")}</span></div>:null}{selected.flex.length?<div><small>OWNED FLEX · {ownedFlex.length}/{selected.flex.length}</small><span>{selected.flex.join(" · ")}</span></div>:null}<p>{raidMeta._meta.note} <a href={raidMeta._meta.sourceUrl} target="_blank" rel="noreferrer">Tacticus Codex</a>, checked {raidMeta._meta.reviewedOn}.</p></section>:null}<DataTable rows={visible} columns={cols} filters={filters} placeholder="Search your character, ability, breakpoint or faction…"/></>;
}
